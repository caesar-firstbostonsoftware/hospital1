...
class Crud_model extends CI_Model {
    ...
    {
        $query = $this->db->query("SELECT bed_id FROM bed_allotment WHERE bed_allotment_id = " . $bed_allotment_id);
        $row = $query->row();
        $returned_array1 = null_checking($data1);
        $this->db->set('status', 'status-1' , FALSE);
        $this->db->where('bed_id',$row->bed_id);
        $this->db->update('bed',$returned_array1);
        //$this->db->where('bed_allotment_id',$bed_allotment_id);
        //$query = $this->db->get();
        //$ret = $query->row();
    ...
    function select_bed_vis()
    {   
        $this->db->order_by('floor', 'ASC');
        return $this->db->get('bed')->result_array();
    }

    function select_bed_floor()
    {
        $this->db->distinct();
    ...
    function save_blood_donor_info()
    {
        ...
        $data1['blood_group']                = $this->input->post('blood_group');
        $data1['status']                        = $this->input->post('volume');
        
$this->db->set('status', 'status+'.$data1['status'] , FALSE);
$this->db->where('blood_group',$data1['blood_group']);
$this->db->update('blood_bank');

/**
$this->db->select('status');
$this->db->where('blood_group',$data1['blood_group']);
$query = $this->db->get('blood_bank');
$data1['status'] = $query->status + $data1['status'];
$returned_array1 = null_checking($data1);
        $this->db->where('blood_group',$data1['blood_group']);
        $this->db->update('blood_bank',$returned_array1);
**/
